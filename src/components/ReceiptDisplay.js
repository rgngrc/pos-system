...
function ReceiptDisplay({ receipt, isReprint, maskId, onNewSale, onReprint }) {
    return (
        <div style={cardStyle}>
            ...
            <button onClick={onNewSale} style={{ width: '100%', padding: '11px', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', fontWeight: 700, fontSize: '13px', cursor: 'pointer', marginBottom: '8px' }}>
                ✓ New Sale
            </button>
            <button onClick={onReprint} style={{ width: '100%', padding: '11px', borderRadius: '10px', border: '1.5px solid #E2E8F0', background: 'white', color: '#1E293B', fontWeight: 600, fontSize: '13px', cursor: 'pointer' }}>
                🔄 Reprint Receipt
            </button>
        </div>
    );
