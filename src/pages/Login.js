...
function Login() {
  const { login, users, loginAttempts, setLoginAttempts, setAuditLog } = useContext(AuthContext);
  ...
  const handleLogin = () => {
    ...
    if (!found) {
      ...
      const lockedUntil = willLock ? new Date(Date.now() + LOCKOUT_MS).toISOString() : null;
      setLoginAttempts(a => ({ ...a, [username]: { count: newCount, lockedUntil } }));
      addAuditEntry(
        willLock
      ...
      if (willLock) {
        ...
        setError(`Invalid username or password. ${MAX_ATTEMPTS - newCount} attempt${MAX_ATTEMPTS - newCount !== 1 ? 's' : ''} remaining.`);
      }
      return;
    }

    if (!found.active) { setError('This account is deactivated. Contact administrator.'); return; }
    ...
    <div style={{
      ...
      <div style={{ background: 'white', borderRadius: '28px', width: '420px', boxShadow: '0 30px 60px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '36px 40px 40px' }}>
          {isLocked && (
            <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: '14px', padding: '16px', textAlign: 'center', marginBottom: '20px' }}>
              <div style={{ fontSize: '28px', marginBottom: '6px' }}>🔒</div>
              <div style={{ fontWeight: 700, color: '#DC2626', fontSize: '14px', marginBottom: '4px' }}>Account Locked</div>
          ...
          {!isLocked && attempts.count > 0 && attempts.count < MAX_ATTEMPTS && (
            <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: '12px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px', fontSize: '12.5px', color: '#92400E' }}>
              ⚠️ {attemptsLeft} attempt{attemptsLeft !== 1 ? 's' : ''} remaining before lockout
            </div>
          )}
          <div style={{ marginBottom: '18px' }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#64748B', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Username</label>
            <input
          ...
          <div style={{ marginBottom: '24px' }}>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#64748B', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Password</label>
            <input
              type="password"
              className="form-control"
              value={password}
          ...
          <button
            ...
            style={{
              width: '100%',
              background: isLocked ? '#E2E8F0' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
